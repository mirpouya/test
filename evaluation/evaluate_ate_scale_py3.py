...
def align(model, data):
    ...
    transGT -- translation vector with scale (3x1)
    trans_errorGT -- translational error with scale per point (1xn)
    trans -- translation vector without scale (3x1)
    trans_error -- translational error without scale per point (1xn)
    s -- scale factor
    """
...
def plot_traj(ax, stamps, traj, style, color, label):
    ...
    Input:
    ax -- the plot
    stamps -- time stamps (list of floats)
    traj -- trajectory (nx3)
    style -- line style
    color -- line color
    label -- plot legend
    """
    # stamps should already be sorted, but we enforce it just in case
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--save',
        help='save aligned second trajectory to disk '
             '(format: stamp2 x2 y2 z2)'
    )
    parser.add_argument(
        '--save_associations',
    ...
    parser.add_argument(
        '--plot',
        help='plot the first and the aligned second trajectory to an image '
             '(format: pdf)'
    )
    parser.add_argument(
        '--verbose',
    ...
    parser.add_argument(
        '--verbose2',
        help='print scale eror and RMSE absolute translational error '
             'in meters after alignment with and without scale correction',
        action='store_true'
    )
    ...
    if len(matches) < 2:
        sys.exit("Couldn't find matching timestamp pairs between groundtruth "
    ...
    # sorted (stamp, data) list for second trajectory
    ...
    if args.verbose:
        ...
        print("absolute_translational_error.std %f m" %
              numpy.std(trans_error))
        print("absolute_translational_error.min %f m" %
              numpy.min(trans_error))
        print("absolute_translational_error.max %f m" %
              numpy.max(trans_error))
        print("max idx: %i" % numpy.argmax(trans_error))
    else:
        # RMSE with scale correction, scale, RMSE with GT scale
        print("%f,%f,%f" % (
            numpy.sqrt(numpy.dot(trans_error, trans_error) / len(trans_error)),
            scale,
            numpy.sqrt(
